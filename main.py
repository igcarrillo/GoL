...
def create_next_grid(rows, cols, grid, next_grid):
    ...
    for row in range(rows):
        for col in range(cols):
            #generating a random number every iteration, between 1 and Alpha
            Alpha = 50
            random_integer = random.randint(1, Alpha)
            ...
            # and the random_integer we generated is not equal to one. So we normally kill, but sometimes let live
            if (live_neighbors < 2 or live_neighbors > 3) and random_integer != 1:
                next_grid[row][col] = 0
            # This exception creates live cells with probability 1/Alpha
            elif (live_neighbors < 2 or live_neighbors > 3) and random_integer == 1:
                next_grid[row][col] = 1
...
def run_game():
    ...
    for gen in range(1, generations + 1):
        ...
        time.sleep(1 / 5000.0)
	#change parameter of sleep to accelerate execution
